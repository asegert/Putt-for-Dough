...
<html class="no-js" lang="">
    <head>
	 ...
	 <title>Dawn of Savings | Sales Event</title>
   ...
   opacity:0;
    ...
    -ms-transform: scale(0.05);
    transform: scale(0.05); 
    text-shadow: 1px 1px 0 #ffffbc; 
}
/* Small devices (tablets, 768px and up) */
@media (min-width: 768px) {
    #congratsOne {font-size: 30px;}
}

/* Medium devices (desktops, 992px and up) */
@media (min-width: 992px) {
   #congratsOne {font-size: 40px;}
}

/* Large devices (large desktops, 1200px and up) */
@media (min-width: 1200px) {
    #congratsOne {font-size: 50px;}
}
        	...
        	<div class="burstBox">
					...
					<div class="row">
						<div class="col-xs-10 col-xs-offset-1 col-md-6 col-md-offset-3">
							<h1 id="congratsOne" class="text-center headline source-bold" style="margin-top:0px;">CONGRATULATIONS</h1>
							<h3 id="discout-phrase-one" class="text-center source-bold">Your score of <span id="game-score">0</span> has unlocked an Instant Win Prize from <%#campaign.GetIt("DealerFriendlyName")%>. You also qualified to win the Grand Prize, a <%# campaign.GetIt("GrandPrize")%></h3>
                        </div>
					</div>
					<div class="row">
						<div class="col-xs-10 col-xs-offset-1 col-md-6 col-md-offset-3 col-lg-4 col-lg-offset-4">
							<img id="hundy" src="http://images.waysideco.ca/dos2016/dos_prize1000.jpg" style="width:100%;height:auto;"/>
						</div>
					</div>
					<div class="row">
						<div class="col-xs-10 col-xs-offset-1 col-md-6 col-md-offset-3">
							<p class="text-center">
								<button type="submit" class="next-btn btn btn-lg btn-success source-bold" style="min-width:30%;">LOCK IN YOUR SCORE</button>
							</p>
						</div>
					</div>
                    <div class="row">
						<div class="col-xs-10 col-xs-offset-1 col-md-6 col-md-offset-3">
							<p class="text-center">
								<button type="button" onClick="playTwo();" class="next-btn btn btn-lg btn-success source-bold" style="min-width:30%;">TRY TO BEAT YOUR SCORE</button>
							</p>
						</div>
					</div>
			</div>
...
<script src="src/start.js"></script>
   ...
   // discountThreeSplit = winThreeSplit.chars,
    nextBtn = document.getElementsByClassName('next-btn');
        ...
        tl.set(discountOneSplit, { transformStyle: "preserve-3d", transformOrigin: "50% 50%", scale: 0.05, opacity: 0 });
        ...
        $("#winOne").on('show.bs.modal', function (e) {
            tl.to(couphundy, 1, { rotation: 1440, opacity: 1, scale: 1, delay: 0.25, ease: Power2.easeIn, delay: 0.25 })
              .to(congratsOne, 0.75, { opacity: 1, scale: 1, ease: Elastic.easeOut.config(0.9, 0.3)}, 1.25)
              .to(discountOneSplit, 0.75, { opacity: 1, scale: 1, ease: Elastic.easeOut.config(0.9, 0.3)}, 1.25)
